...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ''' 
        YOUR CODE: 
        - Load audio from file (recommend to use torchaudio, faster than librosa)
        - Convert it to mono-channel, by simply average the two channels within the audio file
        - Down sample it to self.sampling_rate
        - Compute mel spectrogram. Use n_fft=2048, n_mels=256, f_min=50, f_max=8000. Derive the hop_length from self.sampling_rate and self.frame_size.
        - Extract the desired clip of audio and annotation, from those of the entire song.
        '''
        ...
        ###
        mel_spectrogram = librosa.feature.melspectrogram(y=y, sr=self.sampling_rate, n_fft=2048, hop_length=hop_length)
        mel_spectrogram = librosa.power_to_db(spectrogram, ref=np.max)
        librosa.display.specshow(mel_spectrogram, y_axis='mel', fmin=50, fmax=8000, x_axis='time')
        ...
        # This duration is the offset time of the last note within the song, which is usually a bit smaller than
        # the actual duration of audio file. Please use this variable as the duration of the song.
        ...
        start_frame = int(start_sec * self.sampling_rate / hop_length)
        end_frame = int(end_sec * self.sampling_rate / hop_length)

        spectrogram_clip = mel_spectrogram[:, start_frame:end_frame]
        onset_clip = onset_roll[start_frame:end_frame]
        offset_clip = offset_roll[start_frame:end_frame]
        octave_clip = octave_roll[start_frame:end_frame]
        pitch_class_clip = pitch_roll[start_frame:end_frame]
